...
class Agent:
    ...
    def action(self):
        ...
        # All three parameters are strings. Call the 'look' method on the server, to take a look around the player, without NY parameter (empty string).
        self.gameserver.make_action(str(self.login), str(self.action_name), str(self.parameters))
        self.gameserver.look(self.login)
        ...
        # First 11 elements of the list represent the agent's environment  # and other 11 elements of his surroundings 
...
class AgentRandom(Agent):
     # TODO 7 - This agent will extend from the previous agent and reimplement (override) the 'action' method so that the action will be 'move' and
        # the passed parameter will be one the directions: 'north', 'west', 'south', 'east'.
    # These directions will be randomly selected # (find the appropriate method from the random package).
    def action(self):
...
class AgentLeftRight(Agent):
    ...
    def action(self):
        if len(self.data) > 5 and len(self.data[5]) > 5:
            # Check if there's a barrier in the current direction
            barrier = self.data[5][4] if self.direction == 'west' else self.data[5][6]

            # If there's a barrier, switch direction
            if barrier in ['t', 'o', '~']:
