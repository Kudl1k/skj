...
def find_dangerous_contacts(file: str, max_distance: float):
    ...
    for line in f:
        stripped = line.strip()
        line_stripped = stripped.split()
        records.append(line_stripped)
        ...
        for j in range(len(records)):
            if(records[i][0] == records[j][0]):
                if(records[i] != records[j]): 
                    x = (int(records[i][2]) - int(records[j][2])) ** 2
                    y = (int(records[i][3]) - int(records[j][3])) ** 2
                    z = (int(records[i][4]) - int(records[j][4])) ** 2
                    sqrtxyz = math.sqrt(x + y + z)

                    if(sqrtxyz <= max_distance):
                        temp = (str(records[i][1]),str(records[j][1]))
                        mylist = sorted(temp)
                        tupleDanger = (mylist[0], mylist[1])
                        tmp_dangerous.append(tupleDanger)
                        """print(str(records[i]) + " " + str(records[j]) + " " + str(sqrtxyz)) """   
                        
                    

    

    final_danger = removeDuplicates(tmp_dangerous)
    sorted_by_second = sorted(final_danger, key=lambda tup: tup[0])
...
"""
    ...
    Ukázkové výstupy pro vstupní soubor `contacts.txt` naleznete v souboru `tests.py`.
    """
...
class VaccinationCenter:
    ...
    def __init__(self, r, obs):
        self.rooms = r
        self.observationRooms = obs
        self.currentlyObs = 0
        self.currentlyInRooms = 0
        self.finished = 0

    def accept_patient(self, n):
        try:
            if(self.rooms != 0):
                self.rooms = self.rooms - n
                self.currentlyInRooms =  self.currentlyInRooms + n
        except Exception as ve:
            print(ve)

    def vaccination_room_count(self):
        return self.currentlyInRooms
   
    def waiting_room_count(self):
        return self.currentlyObs
